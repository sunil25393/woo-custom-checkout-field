 ...
 */
/* * ********** constant ddefine ******************* */
define('CCF_VERSION', '1.0.2');

define('CCF_REQUIRED_WP_VERSION', '1.0.2');
...
function ccf_enqueued_style() {
       if (get_option('ccf_plugin_activation')) {
              wp_enqueue_style('bootstrap_css', ccf_plugin_url('css/ccf_custom.css'), array(), CCF_VERSION, 'all');
       }
}
...
function ccf_enqueued_scripts() {
       if (get_option('ccf_plugin_activation')) {
              wp_enqueue_script('ccf_script', plugin_dir_url(__FILE__) . '/js/ccf_scripts.js', array('jquery'), '1.0', true);
       }
}
